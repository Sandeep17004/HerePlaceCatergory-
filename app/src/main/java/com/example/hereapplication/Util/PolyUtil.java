...
import static com.example.hereapplication.Util.MathUtil.mercator;
import static com.example.hereapplication.Util.MathUtil.EARTH_RADIUS;
import static com.example.hereapplication.Util.MathUtil.clamp;
import static com.example.hereapplication.Util.MathUtil.hav;
import static com.example.hereapplication.Util.MathUtil.havDistance;
import static com.example.hereapplication.Util.MathUtil.havFromSin;
import static com.example.hereapplication.Util.MathUtil.inverseMercator;
import static com.example.hereapplication.Util.MathUtil.sinFromHav;
import static com.example.hereapplication.Util.MathUtil.sinSumFromHav;
import static com.example.hereapplication.Util.MathUtil.wrap;
import static com.example.hereapplication.Util.SphericalUtil.computeDistanceBetween;
...
public class PolyUtil {
    ...
    public static boolean containsLocation(GeoCoordinates point, List<GeoCoordinates> polygon, boolean geodesic) {
        ...
        int nIntersect = 0;
        for (GeoCoordinates point2 : polygon) {
    ...
    /**
     * Same as {@link #isLocationOnEdge(GeoCoordinates, List, boolean, double)} with a default tolerance of
     * 0.1 meters.
     */
    public static boolean isLocationOnEdge(GeoCoordinates point, List<GeoCoordinates> polygon, boolean geodesic) {
    ...
    /**
     * Same as {@link #isLocationOnPath(GeoCoordinates, List, boolean, double)}
     * <p>
    ...
    public static boolean isLocationOnEdgeOrPath(GeoCoordinates point, List<GeoCoordinates> poly, boolean closed,
        ...
        double lng1 = toRadians(prev.longitude);
        if (geodesic) {
            for (GeoCoordinates point2 : poly) {
                double lat2 = toRadians(point2.latitude);
                double lng2 = toRadians(point2.longitude);
    ...
    public static List<GeoCoordinates> simplify(List<GeoCoordinates> poly, double tolerance) {
        ...
        if (closedPolygon) {
            ...
            // GeoCoordinates.latitude and .longitude are immutable, so replace the last point
            poly.remove(poly.size() - 1);
            poly.add(new GeoCoordinates(lastPoint.latitude + OFFSET, lastPoint.longitude + OFFSET));
        }
     ...
     */
    public static double distanceToLine(final GeoCoordinates p, final GeoCoordinates start, final GeoCoordinates end) {
        if (start.equals(end)) {
        ...
        GeoCoordinates sb =
                new GeoCoordinates(u * (end.latitude - start.latitude), u * (end.longitude - start.longitude));
        return computeDistanceBetween(sa, sb);
    }

    /**
     * Decodes an encoded path string into a sequence of GeoCoordinatess.
     */
    public static List<GeoCoordinates> decode(final String encodedPath) {
        int len = encodedPath.length();

        // For speed we preallocate to an upper bound on the final length, then
        // truncate the array before returning.
        final List<GeoCoordinates> path = new ArrayList<GeoCoordinates>();
        int index = 0;
        int lat = 0;
        int lng = 0;
        ...
        while (index < len) {
            ...
            path.add(new GeoCoordinates(lat * 1e-5, lng * 1e-5));
        }
    ...
    /**
     * Encodes a sequence of GeoCoordinatess into an encoded path string.
     */
    public static String encode(final List<GeoCoordinates> path) {
        long lastLat = 0;
        long lastLng = 0;

        final StringBuffer result = new StringBuffer();

        for (final GeoCoordinates point : path) {
